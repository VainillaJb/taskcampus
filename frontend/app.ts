...
const API_URL: string = 'http://localhost:5000/tasks';

// Definición de la estructura de una Tarea usando una Interface de TypeScript
interface Task {
    id: string;
    titulo: string;
    asignatura: string;
...
const tasksContainer = document.getElementById('tasks-container') as HTMLDivElement | null;
...
const taskIdInput = document.getElementById('task-id') as HTMLInputElement | null;
const tituloInput = document.getElementById('titulo') as HTMLInputElement | null;
const asignaturaInput = document.getElementById('asignatura') as HTMLInputElement | null;
const descripcionInput = document.getElementById('descripcion') as HTMLTextAreaElement | null;
const fechaInput = document.getElementById('fecha_entrega') as HTMLInputElement | null;
const prioridadInput = document.getElementById('prioridad') as HTMLSelectElement | null;
...
const filterPrioridad = document.getElementById('filter-prioridad') as HTMLSelectElement | null;
const filterAsignatura = document.getElementById('filter-asignatura') as HTMLInputElement | null;
...
async function fetchSummary(): Promise<void> {
    try {
        ...
        if (pendEl) pendEl.innerText = summary.pendientes;
        if (finEl) finEl.innerText = summary.finalizadas;
        if (altaEl) altaEl.innerText = summary.alta_prioridad;
    } catch (e) { 
...
if (taskForm) {
    taskForm.addEventListener('submit', async (e: Event) => {
        ...
        if (!tituloInput || !asignaturaInput || !descripcionInput || !fechaInput || !prioridadInput || !estadoInput || !taskIdInput) return;
        ...
        const id: string = taskIdInput.value;
        const url: string = id ? `${API_URL}/${id}` : API_URL;
...
function renderTasks(): void {
    if (!tasksContainer || !filterEstado || !filterPrioridad || !filterAsignatura) return;
...
function editTask(id: string): void {
    ...
    if (!task || !taskIdInput || !tituloInput || !asignaturaInput || !descripcionInput || !fechaInput || !prioridadInput || !estadoInput || !formTitle || !btnSubmit || !btnCancel) return;
...
function resetForm(): void {
    if (!taskForm || !taskIdInput || !formTitle || !btnSubmit || !btnCancel) return;
    taskForm.reset();
    taskIdInput.value = '';
...
if (filterEstado) filterEstado.addEventListener('change', renderTasks);
if (filterPrioridad) filterPrioridad.addEventListener('change', renderTasks);
if (filterAsignatura) filterAsignatura.addEventListener('input', renderTasks);
